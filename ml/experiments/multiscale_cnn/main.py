"""
Multi-Scale CNN Experiment
===========================

Train multi-scale CNN with parallel branches for different continent sizes.
"""
...
def main():
    ...
    print("=" * 70)
    print("EPIC MULTI-SCALE CNN EXPERIMENT")
    print("=" * 70)
    ...
    # Compare to baseline and ResNet
    print(f"\n📊 Performance Comparison:")
    print(f"  Model               F1 (macro)   Precision   Recall")
    print(f"  " + "-" * 60)
    
    # Baseline
    baseline_path = project_root / "experiments" / "baseline_cnn" / "outputs" / "test_metrics.json"
    if baseline_path.exists():
        with open(baseline_path, 'r') as f:
            baseline = json.load(f)
        print(f"  Baseline CNN        {baseline['f1_macro']:.4f}       {baseline['precision_macro']:.4f}      {baseline['recall_macro']:.4f}")
    
    # Multi-scale (current)
    print(f"  Multi-Scale CNN     {results['f1_macro']:.4f}       {results['precision_macro']:.4f}      {results['recall_macro']:.4f}")
    
    # ResNet
    resnet_path = project_root / "experiments" / "resnet_transfer" / "outputs" / "test_metrics.json"
    if resnet_path.exists():
        with open(resnet_path, 'r') as f:
            resnet = json.load(f)
        print(f"  ResNet18 Transfer   {resnet['f1_macro']:.4f}       {resnet['precision_macro']:.4f}      {resnet['recall_macro']:.4f}")
