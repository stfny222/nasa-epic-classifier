"""
ResNet Transfer Learning Experiment
====================================

Fine-tune pre-trained ResNet18 for continent visibility prediction.
"""
...
def main():
    ...
    print("=" * 70)
    print("EPIC RESNET TRANSFER LEARNING EXPERIMENT")
    print("=" * 70)
    ...
    results = evaluate_geographic_model(
        ...
        threshold=config['training'].get('evaluation_threshold', 0.35)
    )
    ...
    # Compare to baseline
    baseline_metrics_path = project_root / "experiments" / "baseline_cnn" / "outputs" / "test_metrics.json"
    if baseline_metrics_path.exists():
        with open(baseline_metrics_path, 'r') as f:
            baseline = json.load(f)
        
        print(f"\n📊 Comparison to Baseline CNN:")
        print(f"  Metric              Baseline    ResNet    Improvement")
        print(f"  " + "-" * 55)
        print(f"  F1 (macro)          {baseline['f1_macro']:.4f}     {results['f1_macro']:.4f}    {results['f1_macro'] - baseline['f1_macro']:+.4f}")
        print(f"  Precision (macro)   {baseline['precision_macro']:.4f}     {results['precision_macro']:.4f}    {results['precision_macro'] - baseline['precision_macro']:+.4f}")
        print(f"  Recall (macro)      {baseline['recall_macro']:.4f}     {results['recall_macro']:.4f}    {results['recall_macro'] - baseline['recall_macro']:+.4f}")
